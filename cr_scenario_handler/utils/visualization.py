    ...
    if behavior_module_state is not None:
        if config.behavior.use_behavior_planner and config.behavior.visualize_states:
            ...
            if config.behavior.visualization_mode in ["EXTENDED", "FULL"]:
                behavior_text_left += (
                    f"goal_velocity: {str(behavior_module_state.get('goal_velocity'))}\n"
                    f"desired_velocity: {str(behavior_module_state.get('desired_velocity'))}\n"
                    f"velocity: {str(behavior_module_state.get('velocity'))}\n"
                )
            if config.behavior.visualization_mode in ["FULL"]:
                behavior_text_left += (
                    f"TTC: {str(behavior_module_state.get('TTC'))}\n"
                    f"MAX: {str(behavior_module_state.get('MAX'))}\n"
                    ...
                    f"condition_factor: {str(behavior_module_state.get('condition_factor'))}\n"
                    f"lon_dyn_cond_factor: {str(behavior_module_state.get('lon_dyn_cond_factor'))}\n"
                    f"lat_dyn_cond_factor: {str(behavior_module_state.get('lat_dyn_cond_factor'))}\n"
                    f"visual_cond_factor: {str(behavior_module_state.get('visual_cond_factor'))}\n"
                )
